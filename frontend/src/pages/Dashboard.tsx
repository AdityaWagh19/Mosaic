...
export function Dashboard({ nav }: { nav: React.ReactNode }) {
  ...
  return <main className="shell">{nav}<div className="studio"><ControlPanel /><section aria-live="polite">{error ? <div className="notice error" role="alert"><strong>This run could not be completed.</strong><p>{error}</p><button className="btn btn-secondary" onClick={() => void run()}>Try again</button></div> : (!result && isRunning) ? <SimulationLoading /> : !result ? <EmptyState /> : <>
      <header className="run-header"><div><p className="eyebrow">{isRunning ? 'SIMULATION IN PROGRESS' : 'COMPLETED RUN'}</p><h1 ref={resultHeadingRef} tabIndex={-1} style={{ outline: 'none' }}>{isRunning ? 'Simulating interactions...' : result.metrics?.converged ? (result.metrics.termination_reason === 'stationarity' ? 'Diversity stabilized' : 'Consensus reached') : 'Maximum interactions reached'}</h1><p>Run {result.run_id ?? '...'}</p></div><div className="actions">
      <button className="btn btn-secondary" onClick={handleDownloadPDF} disabled={isPdfGenerating || isRunning}>{isPdfGenerating ? 'Generating PDF...' : 'Download PDF'}</button>
      <button className="btn btn-secondary" disabled={isRunning} onClick={() => void copy()}>Copy run link</button><button className="btn btn-secondary" disabled={isRunning} onClick={() => { setConfig(result.config); navigate('/simulate'); }}>Use this configuration</button></div></header>
    <div className="result-statement"><span className="result-statement-label">Run interpretation</span><p>{isRunning ? 'The model is currently running and calculating interactions. Wait for the simulation to finish to see the final interpretation.' : interpretRun(result)}</p></div>
    <div className="metrics"><StaggerFade stagger={0.06}><Metric label="Run status" value={isRunning ? 'Running' : result.metrics?.converged ? (result.metrics.termination_reason === 'stationarity' ? 'Diversity stabilized' : 'Consensus reached') : 'Maximum reached'} help={!isRunning && result.metrics?.converged && result.metrics.termination_reason === 'stationarity' ? "Whether the maximum agent displacement over a sliding window remained below the noise threshold." : "Whether every final accent vector fell within the model's consensus tolerance before the interaction limit."} /><Metric label="Convergence time" value={isRunning ? '...' : result.metrics?.converged ? `${result.metrics.convergence_time} steps` : 'Not reached'} number={isRunning ? undefined : (result.metrics?.converged ? result.metrics.convergence_time : undefined)} help={!isRunning && result.metrics?.converged && result.metrics.termination_reason === 'stationarity' ? "The step at which the population stabilized." : "The first logged step at which all agent accents met the consensus criterion."} /><Metric label={isRunning ? "Current diversity" : "Final diversity"} value={`H = ${(isRunning ? (result.timeline[result.timeline.length - 1]?.diversity ?? 0) : result.metrics?.final_diversity ?? 0).toFixed(3)}`} number={isRunning ? (result.timeline[result.timeline.length - 1]?.diversity ?? 0) : result.metrics?.final_diversity ?? 0} help="Shannon entropy of the accent clusters; it is zero at numerical consensus." /><Metric label={isRunning ? "Current distance" : "Pairwise distance"} value={`D = ${(isRunning ? (result.timeline[result.timeline.length - 1]?.pairwise_distance ?? 0) : result.metrics?.final_pairwise_distance ?? 0).toFixed(3)}`} number={isRunning ? (result.timeline[result.timeline.length - 1]?.pairwise_distance ?? 0) : result.metrics?.final_pairwise_distance ?? 0} help="Average separation among synthetic accent vectors." /></StaggerFade></div>
...
function interpretRun(result: RunResponse) { 
  const { metrics } = result; 
  if (!metrics) return 'The model is currently running and calculating interactions. Wait for the simulation to finish to see the final interpretation.';
  return metrics.converged ? (metrics.termination_reason === 'stationarity' ? `The population stabilized into a noisy equilibrium after ${metrics.convergence_time} steps. Final diversity was ${metrics.final_diversity.toFixed(3)}.` : `All agent accents reached the model's consensus tolerance after ${metrics.convergence_time} steps. Final diversity was ${metrics.final_diversity.toFixed(3)}.`) : `The population did not reach the model's convergence criteria within the ${result.config.T.toLocaleString()}-step limit. Final diversity was ${metrics.final_diversity.toFixed(3)}; inspect the trajectory to distinguish persistent groups from continuing movement.`; 
}
