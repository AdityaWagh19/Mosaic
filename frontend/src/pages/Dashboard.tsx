...
export function Dashboard({ nav }: { nav: React.ReactNode }) {
  ...
  const handleDownloadPDF = async () => {
    ...
    try {
      ...
      const networkSvg = document.querySelector('.network-layout svg') as SVGSVGElement | null;
      const umapSvg = document.querySelector('.stage-selector + .viz svg') as SVGSVGElement | null;
      const timelineSvg = document.querySelector('.recharts-surface') as SVGSVGElement | null;
  ...
  return <main className="shell">{nav}<div className="studio"><ControlPanel /><section aria-live="polite">{error ? <div className="notice error" role="alert"><strong>This run could not be completed.</strong><p>{error}</p><button className="btn btn-secondary" onClick={() => void run()}>Try again</button></div> : isRunning ? <SimulationLoading /> : !result ? <EmptyState /> : <>
      <header className="run-header"><div><p className="eyebrow">COMPLETED RUN</p><h1 ref={resultHeadingRef} tabIndex={-1} style={{ outline: 'none' }}>{result.metrics.converged ? 'Consensus reached' : 'Maximum interactions reached'}</h1><p>Run {result.run_id}</p></div><div className="actions">
    ...
    <div className="metrics"><StaggerFade stagger={0.06}><Metric label="Run status" value={result.metrics.converged ? 'Consensus reached' : 'Maximum reached'} help="Whether every final accent vector fell within the model's consensus tolerance before the interaction limit." /><Metric label="Convergence time" value={result.metrics.converged ? `${result.metrics.convergence_time} steps` : 'Not reached'} number={result.metrics.converged ? result.metrics.convergence_time : undefined} help="The first logged step at which all agent accents met the consensus criterion." /><Metric label="Final diversity" value={`H = ${result.metrics.final_diversity.toFixed(3)}`} number={result.metrics.final_diversity} help="Shannon entropy of the final accent clusters; it is zero at numerical consensus." /><Metric label="Pairwise distance" value={`D = ${result.metrics.final_pairwise_distance.toFixed(3)}`} number={result.metrics.final_pairwise_distance} help="Average separation among synthetic accent vectors." /></StaggerFade></div>
...
function interpretRun(result: RunResponse) { const { metrics } = result; return metrics.converged ? `All agent accents reached the model's consensus tolerance after ${metrics.convergence_time} steps. Final diversity was ${metrics.final_diversity.toFixed(3)}.` : `The population did not reach the model's consensus tolerance within the ${result.config.T.toLocaleString()}-step limit. Final diversity was ${metrics.final_diversity.toFixed(3)}; inspect the trajectory to distinguish persistent groups from continuing movement.`; }
