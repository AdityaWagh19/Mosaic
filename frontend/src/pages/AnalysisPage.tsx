  ...
  return (
    <main className="shell">
      ...
      <header className="hero" style={{ paddingBottom: 32 }}>
        ...
        <BlurReveal as="h1">What the model learned.</BlurReveal>
        <p className="lede">
      ...
      {error ? (
        ...
        <>
          ...
          <section className="panel" style={{ marginBottom: 48, borderLeft: '3px solid var(--color-hairline)' }}>
            ...
            <details style={{ marginTop: 24, padding: '16px 20px', background: 'var(--color-paper)', border: '1px solid var(--color-hairline)', borderRadius: 'var(--radius-cards)' }}>
              <summary style={{ fontWeight: 600, cursor: 'pointer', outline: 'none' }}>What this benchmark does not show (Limitations)</summary>
              <div style={{ marginTop: 12, fontSize: 14, color: 'var(--color-graphite)', display: 'flex', flexDirection: 'column', gap: 12 }}>
                <p style={{ margin: 0 }}><strong>Synthetic Data Only:</strong> The models were trained exclusively on synthetic outputs from the Mosaic accommodation model. The findings do not validate any claims about real-world acoustic or sociolinguistic data.</p>
                <p style={{ margin: 0 }}><strong>Post-hoc Target:</strong> The labels are generated by running k-means (k=5) on the final simulated accent vectors. This creates a classification task, but these classes are not naturally separated "dialects" discovered by the model.</p>
                <p style={{ margin: 0 }}><strong>Scale:</strong> The benchmark relies on {data.n_train_runs} training runs and {data.n_test_runs} test runs. This is sufficient for a demonstration but small for robust ML generalization.</p>
              </div>
            </details>
          </section>
